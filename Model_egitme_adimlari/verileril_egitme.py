...
# İlk iki sütunu X (özellikler) ve son iki sütunu y (hedef) olarak ayırın
X = all_data.iloc[:, :2]
y = all_data.iloc[:, 2:]
...
# Birinci hedef değişkeni görselleştirme
plt.subplot(1, 2, 1)
plt.scatter(y_test.iloc[:, 0], y_pred[:, 0], alpha=0.5)
plt.xlabel('Gerçek Değerler')
plt.ylabel('Tahminler')
plt.title('Birinci Hedef Değişkeni')

# İkinci hedef değişkeni görselleştirme
plt.subplot(1, 2, 2)
plt.scatter(y_test.iloc[:, 1], y_pred[:, 1], alpha=0.5, color='orange')
plt.xlabel('Gerçek Değerler')
plt.ylabel('Tahminler')
plt.title('İkinci Hedef Değişkeni')
